...
unsigned long long statistics::triangles(graph G)
{
    ...
    for (i=0;i<G->n;i++)
    {
        if (G->V[i].edgecount > 1)
        {
            ...
            {
                // only the lowest degree vertex is taken as "central"
                if (G->V[i].edgecount > G->V[G->V[i].sibl[j]].edgecount)
                {
                    continue;
                }
                // ties are broken using the index of the vertex
                if (G->V[i].edgecount == G->V[G->V[i].sibl[j]].edgecount && i < G->V[i].sibl[j])
                {
                    continue;
                }
                for (k=j+1;k<G->V[i].edgecount;k++)
                {
                    // only the lowest vertex is taken as "central"
                    if (G->V[i].edgecount > G->V[G->V[i].sibl[k]].edgecount)
                    {
                        continue;
                    }
                    if (G->V[i].edgecount == G->V[G->V[i].sibl[k]].edgecount && i < G->V[i].sibl[k])
                    {
                        continue;
                    }
...
refer statistics::domset_lb(graph G)
{
    refer max_deg;

    max_deg = max_degree(G);

    return ceil((double) G->n / (double) (max_deg + 1));
}
